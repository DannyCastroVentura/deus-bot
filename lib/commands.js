...
module.exports = (message, args, command, api_key) => {
    const commands = {
        ...
        "inferno": () => {
            ...
            message.mentions.users.first.send("ola");
            // client.fetchUser(message.mentions.users.first, false).then((user) => {
            //     console.log("entrou no fetchUser");
            //     user.send('foste para o inferno');
            // });
        //     setInterval(async function(){
        //          message.channel.send(args[0]+" teve um comportamento inadequado e foi enviado para o inferno", {});} , 10);
        //    
        },
        "biblia": () => {
